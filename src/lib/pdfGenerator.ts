...
export async function generatePDFReport(
  ...
  doc.text(`REPORTE DE ASISTENCIA - FICHA: ${courseData.ficha_de_caracterizacion}`, pageWidth / 2, 18, { align: 'center' });
  ...
  if (reportType === REPORT_TYPES.RIESGO) {
    dataToPrint = studentsWithStats.filter(s => s.enRiesgo || s.enRiesgoTarde);
  }
  ...
  doc.save(`Reporte_Asistencia_Ficha_${courseData.ficha_de_caracterizacion}.pdf`);
}
