...
export default function App() {
  ...
  // Reducido a pestañas esenciales y limpias (eliminada la duplicidad)
  ...
  return (
    <div className="min-h-screen bg-slate-50 text-slate-900 font-sans pb-12 flex flex-col justify-between">
      ...
      <main className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 mt-6 space-y-6 w-full flex-grow">
        ...
        {/* Menú principal simplificado y unificado (sin redundancias) */}
        ...
        {/* PESTAÑA PRINCIPAL UNIFICADA: Tablero KPI + Matriz Cronológica de Asistencia */}
      ...
      {/* Modal para Tomar Asistencia Diaria */}
      {showAttendanceModal && (
        <div className="fixed inset-0 bg-slate-900/50 backdrop-blur-sm z-50 flex items-center justify-center p-4">
          <div className="bg-white rounded-2xl shadow-2xl border border-slate-200 w-full max-w-3xl overflow-hidden flex flex-col max-h-[90vh]">
            <div className="p-4 sm:p-6 border-b border-slate-100 flex items-center justify-between bg-slate-50">
              <div className="flex items-center gap-3">
                <div className="p-2.5 bg-emerald-100 text-sena rounded-xl">
                  <ClipboardList className="w-6 h-6" />
                </div>
                <div>
                  <h3 className="text-lg font-bold text-slate-800">Registro Diario de Asistencia</h3>
                  <p className="text-xs text-slate-500">Ficha {courseData.ficha_de_caracterizacion} - {courseData.denominacion}</p>
                </div>
              </div>
              <button 
            ...
            <div className="p-4 sm:p-6 border-b border-slate-100 flex items-center justify-between gap-4 bg-slate-50/50">
              <label className="text-xs font-bold text-slate-700 uppercase tracking-wide">Fecha de la Sesión:</label>
              <input 
                type="date"
                value={attendanceDate}
                onChange={(e) => setAttendanceDate(e.target.value)}
                className="border border-slate-300 rounded-lg px-3 py-1.5 text-sm font-bold bg-white text-slate-800 shadow-sm focus:outline-none focus:ring-2 focus:ring-sena"
              />
            </div>

            <div className="p-4 sm:p-6 overflow-y-auto flex-grow divide-y divide-slate-100">
              {courseData.asistencias_aprendices.map((student) => {
                const currentStatus = tempRecords[student.numero_documento] || 'Presente';
                return (
                  <div key={student.numero_documento} className="py-3 flex flex-col sm:flex-row sm:items-center justify-between gap-3 hover:bg-slate-50 px-3 rounded-lg transition-colors">
                    <div>
                      <p className="font-bold text-slate-800 text-sm">{student.apellidos} {student.nombres}</p>
                      <p className="font-mono text-xs text-slate-400">{student.numero_documento}</p>
                    </div>
                    <div className="flex items-center gap-1.5 bg-white border border-slate-200 p-1 rounded-lg shadow-xs">
                      {['Presente', 'X', 'Tarde', 'Excusa'].map((statusOption) => {
                        const isSelected = currentStatus === statusOption;
                        let btnColor = '';
                        if (isSelected) {
                          if (statusOption === 'Presente') btnColor = 'bg-emerald-600 text-white font-bold shadow-sm';
                          else if (statusOption === 'X') btnColor = 'bg-red-600 text-white font-bold shadow-sm';
                          else if (statusOption === 'Tarde') btnColor = 'bg-amber-500 text-white font-bold shadow-sm';
                          else if (statusOption === 'Excusa') btnColor = 'bg-blue-600 text-white font-bold shadow-sm';
                        } else {
                          btnColor = 'text-slate-600 hover:bg-slate-100';
                        }
                        
                        let label = statusOption;
                        if (statusOption === 'X') label = 'Falta (X)';

                        return (
                          <button
                            key={statusOption}
                            type="button"
                            onClick={() => setTempRecords({ ...tempRecords, [student.numero_documento]: statusOption })}
                            className={`px-3 py-1 text-xs rounded-md transition-all ${btnColor}`}
                          >
                            {label}
                          </button>
                        );
            ...
            <div className="p-4 sm:p-6 border-t border-slate-100 bg-slate-50 flex items-center justify-end gap-3">
              ...
              <button
                ...
                className="px-5 py-2 bg-sena text-white rounded-lg text-sm font-semibold hover:bg-sena-dark shadow-sm transition-colors flex items-center gap-2"
              >
